...
@app.on_message(filters.private & filters.command(["logger"]) & SUDOERS & LOVE)
@language
...
@app.on_message(filters.group & filters.command(["logger"]) & SUDOERS & LOVE)
@language
async def logger_group(client, message, _):
    if not message.from_user.id in SUDOERS:
        return
    usage = _["log_1"]
    if len(message.command) != 2:
        return await
